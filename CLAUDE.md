...
- **`internal/config`** — `config.toml`, **hand-authored per device**. Maps a logical `name` to this machine's absolute `path`. Never synced between devices.
...
2. **State classification drives every refusal.** `classify()` in `status.go` compares both fingerprints against `state.json` to produce clean/ahead/behind/conflict. `push` refuses on behind/conflict, `pull` on ahead/conflict, unless `--force`. `link` deliberately records *only* `gist_id` — writing sync markers there would make the next `pull` think the sides already match.
